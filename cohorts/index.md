...
<section class="mb-12 space-y-4">
  <span class="text-xs font-semibold uppercase tracking-[0.3em] text-brand-stone/60">Gallery</span>
  <h1 class="text-4xl font-semibold text-brand-navy sm:text-5xl">Cohort archive</h1>
  <p class="max-w-2xl text-lg leading-relaxed text-brand-stone/80">Browse every published DIVE cohort. Each year includes a timeline, resource kits, and a searchable gallery of project teams.</p>
</section>
  ...
  {% for cohort in sorted_years reversed %}
    ...
    <a class="group relative overflow-hidden rounded-2xl border border-brand-sky/15 bg-white/90 p-6 shadow-subtle transition hover:-translate-y-1 hover:border-brand-sky/40 hover:shadow-xl" href="{{ '/cohorts/' | append: year | append: '/' | relative_url }}">
      ...
      <div class="flex items-center justify-between gap-4">
        ...
        <span class="inline-flex items-center gap-2 rounded-full bg-brand-sky/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-brand-sky">{{ teams }} teams</span>
      </div>
      {% if kickoff %}
        <h2 class="mt-4 text-xl font-semibold text-brand-navy">{{ kickoff.name }}</h2>
        <p class="mt-1 text-sm text-brand-stone/70">Kickoff {{ kickoff.date | date: "%B %-d, %Y" }}</p>
      {% else %}
