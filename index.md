...
<section class="relative mb-16 overflow-hidden rounded-3xl border border-brand-sky/20 bg-gradient-to-br from-white via-brand-sky/5 to-brand-sea/5 shadow-card">
  ...
  <div class="relative grid gap-12 px-6 py-14 sm:px-10 lg:grid-cols-[1.6fr_minmax(0,1fr)]">
    <div class="space-y-6">
      <span class="inline-flex items-center gap-2 rounded-full bg-white/60 px-4 py-2 text-xs font-semibold uppercase tracking-[0.32em] text-brand-stone/70 shadow-subtle">City of Austin · DIVE Program</span>
      <h1 class="text-4xl font-semibold text-brand-navy sm:text-5xl">Data Learning Cohorts Gallery</h1>
      <p class="max-w-2xl text-lg leading-relaxed text-brand-stone/90">Discover how Austin public servants apply data science, storytelling, and civic analytics to deliver measurable impact. Each cohort highlights cross-department teams, curated resources, and outcomes you can explore and reuse.</p>
      <div class="flex flex-col gap-3 sm:flex-row">
        ...
        <a class="inline-flex items-center justify-center rounded-full border border-brand-sky/40 bg-white/70 px-6 py-3 text-sm font-semibold text-brand-sky transition hover:-translate-y-0.5 hover:bg-white focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-sky/30" href="{{ '/cohorts/' | relative_url }}">
          View Cohort Archive
        </a>
      </div>
    </div>
...
<section class="mb-16 space-y-8">
  <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
    <div>
      ...
      <p class="text-sm text-brand-stone/70">Highlights from the {{ featured_year }} cohort demonstrating civic data leadership.</p>
    </div>
  ...
  <div class="grid gap-6 lg:grid-cols-2">
    ...
    {% else %}
      <div class="rounded-2xl border border-brand-sky/15 bg-white/80 p-8 text-sm text-brand-stone/80 shadow-subtle">
...
<section class="mb-16 space-y-8">
  <div>
    ...
    <p class="text-sm text-brand-stone/70">Program insights illustrating how teams leverage data, storytelling, and community partnerships.</p>
  </div>
  <div class="grid gap-6 md:grid-cols-3">
    <div class="rounded-2xl border border-brand-sky/10 bg-white/80 p-6 shadow-subtle">
      <span class="text-sm font-semibold uppercase tracking-[0.3em] text-brand-stone/50">Civic Storytelling</span>
      <h3 class="mt-4 text-xl font-semibold text-brand-navy">Data narratives that resonate</h3>
      <p class="mt-3 text-sm text-brand-stone/80">Teams translate complex datasets into accessible dashboards and posters that help residents and leaders make informed decisions.</p>
    </div>
    <div class="rounded-2xl border border-brand-sky/10 bg-white/80 p-6 shadow-subtle">
      <span class="text-sm font-semibold uppercase tracking-[0.3em] text-brand-stone/50">Cross-Department</span>
      <h3 class="mt-4 text-xl font-semibold text-brand-navy">Collaborative delivery</h3>
      <p class="mt-3 text-sm text-brand-stone/80">Coaches and analysts from multiple departments pair up to co-design solutions, strengthening citywide data literacy.</p>
    </div>
    <div class="rounded-2xl border border-brand-sky/10 bg-white/80 p-6 shadow-subtle">
      <span class="text-sm font-semibold uppercase tracking-[0.3em] text-brand-stone/50">Re-usable Assets</span>
      <h3 class="mt-4 text-xl font-semibold text-brand-navy">Open resources for future teams</h3>
      <p class="mt-3 text-sm text-brand-stone/80">Each project publishes deliverables, playbooks, and accessibility notes to accelerate future cohorts and community partners.</p>
    </div>
  </div>
</section>
...
<section class="space-y-8">
  <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
    <div>
      ...
      <p class="text-sm text-brand-stone/70">Select a year to view teams, schedules, and learning resources.</p>
    </div>
    <span class="text-xs font-medium uppercase tracking-[0.3em] text-brand-stone/50">{{ total_cohorts }} published cohorts</span>
  </div>
    ...
    {% for cohort in cohorts reversed %}
      ...
      <a class="group relative overflow-hidden rounded-2xl border border-brand-sky/10 bg-white/90 p-6 shadow-subtle transition hover:-translate-y-1 hover:border-brand-sky/40 hover:shadow-xl" href="{{ '/cohorts/' | append: year | append: '/' | relative_url }}">
        ...
        <div class="flex items-center justify-between gap-4">
          <span class="text-xs font-semibold uppercase tracking-[0.3em] text-brand-stone/60">Cohort {{ year }}</span>
          <span class="inline-flex items-center gap-2 rounded-full bg-brand-sky/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-brand-sky">{{ teams_for_year }} teams</span>
        </div>
        {% if event %}
          <h3 class="mt-4 text-xl font-semibold text-brand-navy">{{ event.name }}</h3>
          <p class="mt-1 text-sm text-brand-stone/70">Kickoff {{ event.date | date: "%B %-d, %Y" }}</p>
        {% endif %}
        <div class="mt-5 flex flex-wrap gap-2 text-[11px] font-medium uppercase tracking-wide text-brand-stone/60">
